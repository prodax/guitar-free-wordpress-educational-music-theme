...
$self_id = guitar_main(get_the_ID(), false);
get_header();
?>
<?php while (have_posts()) : the_post(); ?>

    <!-- Intro Content begns -->
    <div id="<?php echo esc_attr(guitar_sectionID(guitar_main($post->ID, false))); ?>" class="onepage-home ">
        <?php the_content(); ?>
    </div>
...
if (isset($menu_locations['primary'])) :
    ...
    foreach ($menu_items as $item) {
        if (($item->object == 'page') && (guitar_get_post_meta(guitar_main($item->object_id, false), 'xs_page_section') == "on") && ($item->object_id != $self_id))
    ...
    if ($query->have_posts()):
        while ($query->have_posts()):
            $query->the_post();
            $section_id = guitar_sectionID(guitar_main($post->ID, false));
            ?>
            <div id="<?php echo esc_attr($section_id); ?>" class="section-wraper">
                <?php the_content(); ?>

                <!-- Footer edit section -->
                <?php echo guitar_edit_section(); ?>
            </div>
